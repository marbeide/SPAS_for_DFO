...
SPAS2.input.file <- function(appdat, recdat, app.strata, combine.app,
  ...
  m.t.rec <- appdat %>%
   ...
   if(combine.app == TRUE & m.t.rec[nrow(m.t.rec),1] == "NA_NA") m.t.rec[nrow(m.t.rec),1] <- NA

    add <- setdiff(m.cor.app[,1], data.frame(app.strata = na.omit(m.t.rec[,1])))
    num <- nrow(add)
    if(num >0) m.t.rec[(nrow(m.t.rec)+1):(nrow(m.t.rec)+num), 1] <- add
    m.t.rec <- arrange(m.t.rec, !!app.strata)
  
    m.t.rec <- as.data.frame(m.t.rec)
  ...
  # takes the proportion column from m.cor.app and uses matrix multiplication to distribute the number of secondary recoveries from each t strata to a t by s matrix
  if(is.na(m.t.rec[nrow(m.t.rec),1])) secondary.add <- as.matrix(m.cor.app[,3]) %*% 
  ...
  m.spasR <- cbind( m.t.rec[!is.na(m.t.rec[,1]), -1],  m.cor.app[ , m.cor.app$applied ] - rowSums(m.t.rec[!is.na(m.t.rec[,1]), -1]))
  m.spasR <- as.matrix(rbind(m.spasR, c(t(m.unt.rec[ , -1]), 0)))
  
  write.table(m.spasR, paste("SPAS_R_", name, "_input_Males.csv", sep = ""), 
              row.names = F, col.names = F, sep = ",")
  write.table(m.cor.app[,1], paste("SPAS_R_", name, "_ROW_STRATA_NAMES.csv", sep = ""), sep = ",")
  ...
  f.t.rec <- appdat %>%
   ...
   if(combine.app == TRUE & f.t.rec[nrow(f.t.rec),1] == "NA_NA") f.t.rec[nrow(f.t.rec),1] <- NA

  
    add <- setdiff(f.cor.app[,1], data.frame(app.strata = na.omit(f.t.rec[,1])))
    num <- nrow(add)
    if(num >0) f.t.rec[(nrow(f.t.rec)+1):(nrow(f.t.rec)+num), 1] <- add
    f.t.rec <- arrange(f.t.rec, !!app.strata)
  
    f.t.rec <- as.data.frame(f.t.rec)
  ...
  if(jacks == TRUE) j.t.rec <- appdat %>%
    ...
    summarise(tag.recoverd = n()) %>% # this creates a long form version
    spread(!!rec.strata, tag.recoverd) # this changes it to the wide form t by s matrix
     if(jacks == TRUE & combine.app == TRUE) if(j.t.rec[nrow(j.t.rec),1] == "NA_NA") j.t.rec[nrow(j.t.rec),1] <- NA

    if(jacks == TRUE) add <- setdiff(j.cor.app[,1], data.frame(app.strata = na.omit(j.t.rec[,1])))
    if(jacks == TRUE) num <- nrow(add)
    if(jacks == TRUE & num >0) j.t.rec[(nrow(j.t.rec)+1):(nrow(j.t.rec)+num), 1] <- add
    if(jacks == TRUE) j.t.rec <- arrange(j.t.rec, !!app.strata)
    if(jacks == TRUE) j.t.rec <- as.data.frame(j.t.rec)
      # need to fill in if any recovery spot in the app strata had a 0, all of the subsetting is to ignore if there are NAs in the app_strata column as it is used as a check to apply secondary mark only recoveries
  ...
  if(jacks == TRUE) if(is.na(j.t.rec[nrow(j.t.rec),1])) j.t.rec[!is.na(j.t.rec[,1]), 
                                              2:ncol(j.t.rec)] <- j.t.rec[!is.na(j.t.rec[,1]), 2:ncol(j.t.rec)] + secondary.add
  ...
  if(jacks == TRUE) write.table(j.spasR, paste("SPAS_R_", name, "_input_Jacks.csv", sep = ""), row.names = F, col.names = F, sep = ",")
