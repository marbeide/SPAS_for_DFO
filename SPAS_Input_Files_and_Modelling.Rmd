...
This Rmarkdown file contains a function that produces the SPAS2 (now SPAS package in R) input file from the MACH2 xlsm file WB5PopEstimator.
...
Row s+1 = contains the total number of UNMARKED animals recovered in column stratum its in
All of the other cells are the number of recovered MARKED individuals of stratum s by t

!!!For first time .Rmd users!!!
To Run code, either click the green "play" button for a given "chunk"" or click to the line you want to run and click "Run" at the top of this pane or click crt-enter (if on a pc, cmd-enter on a mac)
...
Since the files are probably all in different places per project-year, the easiest way to set this is likely by going into the "Session" tab, "Set Working Directory", "Choose Directory..."
...
The next chunk renames those annoying strata column names with = and ' and other symbols that confuse R
...
# if all are TRUE, you do not need to modify the below, if FALSE, you will have to find the correct positions
```
...
Once you run the chunk, you can minimize the code using the little arrow beside the first line (93) in the chunk (just so there is less scrolling to do)
