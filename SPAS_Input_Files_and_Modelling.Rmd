...
SPAS2.input.file <- function(appdat, recdat, app.strata, combine.app,
                              ...
                              sex.correction, name, jacks, square){ 
...
```{r}
mod..1 <- SPAS.fit.model(males, # make sure data frame is correct
...
mod..2 <- SPAS.fit.model(males, # make sure data frame is correct
...
write.xlsx(report.males, paste(NAME, "_males_report.xlsx", sep = ""), row.names = F)
# can ignore "Note", likely from packages not all being up to date.
```

## SPAS for Females
```{r}
mod..1 <- SPAS.fit.model(females, # make sure data frame is correct
                           model.id = "Petersen", # change name to desired description
                       row.pool.in = c(1, 1), # add ,1 if its a 3 x 3
                       # REMEMBER - can only pool rows, NOT columns
                       col.pool.in = c(1, 2), # add ,3 if its a 3 x 3
...
mod..2 <- SPAS.fit.model(females, # make sure data frame is correct
                             model.id = "Stratified", # change name to desired description
                       row.pool.in = c(1, 2), # add ,3 if its a 3 x 3
                       # REMEMBER - can only pool rows, NOT columns
                       col.pool.in = c(1, 2), # add ,3 if its a 3 x 3
...
write.xlsx(report.females, paste(NAME, "_females_report.xlsx", sep = ""), row.names = F)
```
